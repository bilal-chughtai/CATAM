...
def integrate(t,r,theta,phi,tprime,rprime,thetaprime,phiprime, m0, a0, Gamma):
    ...
    def vectorfield(w, tau, p):
        ...
        #compute DE for yi:
        yprime=defaultdict(float)
        for i in range(1,5):
            for j in range (1,5):
                for k in range (1,5):
                    yprime[i] += (-Gamma(-i,j,k)).subs([(t,x1),(r,x2),(theta,x3), (phi,x4), (m,m0), (a,a0)])*y[j-1]*y[k-1] #-1 as python list

        # Create f = (x1',y1',x2',y2'):
        f = [y1,yprime[1],y2,yprime[2],y3,yprime[3],y4,yprime[4]]
        return f
